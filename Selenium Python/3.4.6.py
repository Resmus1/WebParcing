# https://stepik.org/lesson/731861/step/9?unit=733396

# Импортируем необходимые модули из библиотеки Selenium
from selenium import webdriver
from selenium.webdriver.common.by import By

# Список чисел, по которым будет производиться проверка
...
with webdriver.Chrome() as browser:
    ...
    # Проходим по каждому найденному чекбоксу
    for item in check_box:
        # Получаем значение чекбокса и преобразуем его в целое число
        value = int(item.get_attribute('value'))
        # Если значение чекбокса есть в списке numbers, кликаем по нему
        item.click() if value in numbers else None  # Используем None вместо 0 для улучшения читаемости
